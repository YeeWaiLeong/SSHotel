...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<h5 class="card-title"><?php echo isset($id) ? "Manage" : "Create" ?> Materials</h5>
		</div>
		<div class="card-body">
			<form id="client">
				<div class="row" class="details">
					...
					<div class="col-sm-6">
						<div class="form-group">
							<label for="" class="control-label">Material Name</label>
				...
				<div class="form-group">
					<label for="" class="control-label">Material Image</label>
				...
				<div class="form-group">
					<label for="" class="control-label">Choose File</label>
					<div class="custom-file">
						<input type="hidden" name="old_file_download" value="<?php echo isset($file_path) ? $file_path :'' ?>">
		              <input type="file" class="custom-file-input rounded-circle" id="customFile2" name="file" onchange="displayFile(this)">
		              <label class="custom-file-label" for="customFile2">Choose file</label>
		            </div>
				</div>
				<div class="form-group d-flex justify-content-center">
					<p id="displayFile" src="<?php echo validate_file(isset($file_path) ? $file_path :'') ?>" alt="" id="cfile"></p>
				</div>
			</form>
		</div>
		<div class="card-footer">
			<button class="btn btn-primary btn-sm" form="client"><?php echo isset($_GET['id']) ? "Update": "Save" ?></button>
			<a class="btn btn-primary btn-sm" href="./?page=materials">Cancel</a>
		</div>
	</div>
</div>
...
<script>
	...
	function displayFile(input) {
		...
		document.getElementById('displayFile').innerHTML = input.files[0]['name'];
	    //window.alert(input.files[0]['name']);
	}
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"admin/?page=materials";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
