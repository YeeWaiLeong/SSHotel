...
Class Content extends DBConnection {
	...
	public function client(){
		...
		$data = "";
		foreach($_POST as $k => $v){
			if(!in_array($k,array('id','description','old_file','old_file_download'))){
				if(!empty($data)) $data .= ", ";
				$data .= "`$k` = '$v'";
			}
		}
		...
		if(isset($_FILES['file']) && $_FILES['file']['tmp_name'] != ''){
			$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['file']['name'];
			$move2 = move_uploaded_file($_FILES['file']['tmp_name'],base_app. $fname);
			if($move2){
				$data .=" , file_path_download = '{$fname}' ";
			}
		}
		...
		if($save){
			...
			if(isset($move2) && $move2 && !empty($old_file)){
				if(is_file(base_app.$old_file))
					unlink(base_app.$old_file);
			}
			$resp['status']='success';
